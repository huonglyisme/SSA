...
def ssa_basic_phase_search(G, H_true, W_T, W_T_1, U_R, U_R_1, M, L, N, K, pop=30, iters=200, seed=None):
    if seed is not None:
        np.random.seed(seed)

    #khởi tạo ngẫu nhiên
    #pop: số cá thể
    #n_vars: tổng số pha cần tối ưu
    n_vars = M*L + N*K
    salps = np.random.rand(pop, n_vars) * 2 * np.pi

    #mảng lưu giá trị nmse của từng cá thể
    nmse_vals = np.zeros(pop)
    #vòng lặp tính nmse cho từng cá thể
    for i in range(pop):
        ph = salps[i]

        #lấy phần pha TX, reshape thành ma trận MxL
        phase_tx = np.exp(1j * ph[:M*L].reshape(M, L))
        #lấy phần pha RX, reshape thành ma trận NxK
        phase_rx = np.exp(1j * ph[M*L:].reshape(N, K))
        #np.exp(1j * ...):chuyển từ góc pha (radian) sang số phức trên đơn vị tròn

        #P = tổng hợp pha TX qua các lớp SIM + ma trận liên tầng W_T
        P = build_P_from_tx_phases(phase_tx, W_T_1, W_T, L)
        #Q = tổng hợp pha RX qua các lớp SIM + ma trận liên tầng U_R
        Q = build_Q_from_rx_phases(phase_rx, U_R_1, U_R, K)
        #tính nmse cho cá thể
        #->trả về beta, nmse, H_SIM(kênh tổng thể)
        _, nmse_vals[i], _ = compute_factor_and_nmse(Q, G, P, H_true)

    #chọn leader ban đầu
    #np.argmax tìm giá trị lớn nhất ->dùng fitness = -nmse_vals
    fitness = -nmse_vals

    #index của cá thể có nmse nhỏ nhất (cá thể tốt nhất)
    best_idx = np.argmax(fitness)
    #best → vector pha của leader
    best = salps[best_idx].copy()
    #best_nmse → NMSE tương ứng của leader
    best_nmse = nmse_vals[best_idx]

    #iters = số vòng lặp tối ưu
    #c1 = bước nhảy của leader, giảm theo thời gian
    for t in range(1, iters+1):
        ...
        for i in range(pop):
            ...
            #cập nhật leader
            #best = pha tốt nhất hiện tại (cá thể leader)
            #step = c1 * c2 → bước nhảy ngẫu nhiên
            #direction = ±1 → ngẫu nhiên chọn hướng
            if i == 0:
                c2 = np.random.rand(n_vars)
                c3 = np.random.rand(n_vars)
                step = c1 * c2
                direction = np.where(c3 < 0.5, 1.0, -1.0)
                move = best + step * direction
                #np.mod(..., 2*np.pi) → wrap pha về [0,2π].
                new = np.mod(move, 2*np.pi)
            else:
                #cập nhật followers
                new = 0.5 * (salps[i] + salps[i-1])
                new = np.mod(new, 2*np.pi)
            new_salps[i] = new

        #tính nmse cho mỗi cá thể mới
        #chọn leader mới = cá thể có nmse nhỏ nhất
        ...
        salps = new_salps
        fitness_new = -nmse_new
        idx = np.argmax(fitness_new)
    ...
    #ph = leader cuối cùng
    ph = best
    #chia ph thành pha TX và RX, đưa về số phức, reshape về đúng kích thước M x L cho TX, N x K cho RX
    phase_tx = np.exp(1j * ph[:M*L].reshape(M, L))
    phase_rx = np.exp(1j * ph[M*L:].reshape(N, K))
    #xây dựng P, Q cuối cùng
    #P = ma trận tổng hợp pha TX cuối cùng qua tất cả lớp SIM + ma trận liên tầng W_T
    P = build_P_from_tx_phases(phase_tx, W_T_1, W_T, L)
    #Q = ma trận tổng hợp pha RX cuối cùng qua tất cả lớp SIM + ma trận liên tầng U_R
    Q = build_Q_from_rx_phases(phase_rx, U_R_1, U_R, K)
    #Tính beta, NMSE và kênh H_SIM cuối cùng
