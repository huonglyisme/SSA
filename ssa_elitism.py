...
def ssa_elitism_phase_search(
    G, H_true, W_T, W_T_1, U_R, U_R_1,
    M, L, N, K,
    pop=30, iters=200, elite_size=3, seed=None
):
    if seed is not None:
        np.random.seed(seed)
    ...
    for t in range(1, iters+1):
        ...
        # ---- ELITISM ----
        elite_idx = np.argsort(nmse_vals)[:elite_size]
        elites = salps[elite_idx]
